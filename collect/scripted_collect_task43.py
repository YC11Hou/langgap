...
Task 43: pick up the black bowl between the plate and the ramekin and place it on the stove.
...
def discover_names(env, debug=False):
    """Find MuJoCo body name for akita_black_bowl_1 and stove cook_region site."""
    ...
    # Find akita_black_bowl_1 body (not bowl_2)
    bowl_body = None
    for n in body_names:
    ...
    # Find stove cook_region site
    stove_site = None
    for n in site_names:
        if "stove" in n.lower() and "cook" in n.lower():
            stove_site = n
            break

    # Fallback: search for stove site
    if stove_site is None:
        for n in site_names:
            if "stove" in n.lower():
                stove_site = n
                break
    ...
    if stove_site is None:
        raise RuntimeError(f"Could not find stove cook site in: {site_names}")

    return bowl_body, stove_site
...
def build_place_waypoints(place_xy, stove_z, init_rot, place_offset=0.02):
    """
    Build place waypoints (5-8): move above, descend, open, lift.

    place_xy is adjusted so the bowl center (not gripper) lands on stove center.
    Each waypoint: (target_pose_4x4, gripper, dist_threshold, hold_steps)
    """
    px, py = place_xy
    sz = stove_z
    ...
    return [
        # 5. Move above stove (gripper closed) - relaxed threshold for long travel
...
def collect_one_demo(env, bowl_body, stove_site, max_steps, debug=False,
    ...
    stove_cook_pos = get_site_pos(env, stove_site)
    ...
    place_xy = stove_cook_pos[:2] - grasp_offset_xy  # bowl center lands on stove center

    if debug:
        print(f"\n  Grasp offset (bowl-eef): [{grasp_offset_xy[0]:.4f}, {grasp_offset_xy[1]:.4f}]")
        print(f"  Adjusted place XY: [{place_xy[0]:.4f}, {place_xy[1]:.4f}] "
              f"(stove center: [{stove_cook_pos[0]:.4f}, {stove_cook_pos[1]:.4f}])")

    place_waypoints = build_place_waypoints(place_xy, stove_cook_pos[2], init_rot,
...
EXPECTED_MAX_STEPS = 105  # Calibrated: max=98, ceil/5×5+5=105

def main():
    parser = argparse.ArgumentParser(
        description="Scripted waypoint demo collection (Task 43: black bowl → stove)")
    ...
    parser.add_argument("--place_offset", type=float, default=0.02,
                        help="Z offset above stove for place (tune this)")
    ...
    print("=" * 60)
    print("Scripted Waypoint Demo Collection (Task 43)")
    print("=" * 60)
    ...
    env.reset()
    bowl_body, stove_site = discover_names(env, debug=args.debug)
    print(f"  Bowl body:   {bowl_body}")
    print(f"  Stove site:  {stove_site}")

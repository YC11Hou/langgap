...
Task 78: put the cream cheese on top of the cabinet.
Strategy: center top-down grasp with fixed GRASP_ROT orientation.
Based on Task 54 (cream cheese → stove), modified target to cabinet top_side.
Cabinet top is at z≈1.127, needs higher lift (0.28).
"""
...
def discover_names(env, debug=False):
    """Find MuJoCo body name for cream cheese and cabinet top_side site."""
    ...
    if cabinet_site is None:
        raise RuntimeError(f"Could not find cabinet top_side site in: {site_names}")

    return cheese_body, cabinet_site
...
def build_waypoints(object_pos, cabinet_pos, place_offset=0.02, grasp_offset=0.01):
    """
    Build waypoint sequence for center top-down grasp, place on cabinet top.

    Cabinet top_side is at z≈1.127 (above table z=0.900).
    Needs higher lift (0.28) to clear cabinet top.
    """
    ox, oy, oz = object_pos
    px, py, pz = cabinet_pos
    ...
    waypoints = [
        # 1. Move above cream cheese center (gripper open)
        ...
        # 4. Lift up high (gripper closed) — must clear cabinet top z≈1.127
        (make_pose(np.array([grasp_x, grasp_y, grasp_z + 0.28]), GRASP_ROT), +1, 0.01, 0),
        # 5. Move above cabinet top (gripper closed)
        (make_pose(np.array([place_x, place_y, pz + 0.10]), GRASP_ROT), +1, 0.015, 0),
        # 6. Descend to place position (gripper closed)
        (make_pose(np.array([place_x, place_y, pz + place_offset]), GRASP_ROT), +1, 0.015, 0),
        # 7. Open gripper (release, hold 15 steps — let cream cheese settle on cabinet)
        (make_pose(np.array([place_x, place_y, pz + place_offset]), GRASP_ROT), -1, 999, 15),
    ]
    return waypoints


def collect_one_demo(env, cheese_body, cabinet_site, max_steps, debug=False,
                     place_offset=0.02):
    ...
    if debug:
        ...
        print(f"  Cabinet top pos:       {cabinet_pos}")
        print(f"  Object Z:             {object_pos[2]:.4f}")
        print(f"  Cabinet Z:              {cabinet_pos[2]:.4f}")
        print(f"  Table top Z:          {table_top_pos[2]:.4f}")

    waypoints = build_waypoints(object_pos, cabinet_pos, place_offset=place_offset)
    ...
    for wp_idx, (target_pose, gripper, dist_thresh, hold_steps) in enumerate(waypoints):
        ...
        if debug:
            ...
            obj_after = get_body_pos(env, cheese_body)
...
EXPECTED_MAX_STEPS = 125  # Calibrated: max=118, ceil/5×5+5=125

def main():
    parser = argparse.ArgumentParser(
        description="Scripted waypoint demo collection (Task 78: cream cheese → stove)")
    ...
    print("=" * 60)
    print("Scripted Waypoint Demo Collection (Task 78)")
    print("=" * 60)
    ...
    env.reset()
    cheese_body, cabinet_site = discover_names(env, debug=args.debug)

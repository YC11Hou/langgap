...
Task 44: pick up the black bowl between the plate and the ramekin and place it on the cabinet.
...
def get_target_bowl_from_bddl(bddl_path: str) -> str:
    """Parse BDDL obj_of_interest to find which akita_black_bowl to pick."""
    with open(bddl_path, "r") as f:
        content = f.read()
    match = re.search(r"\(:obj_of_interest\s+(.*?)\)", content, re.DOTALL)
    if match:
        objects = match.group(1).strip().split()
        for obj in objects:
            if "akita_black_bowl" in obj:
                return obj
    return "akita_black_bowl_1"  # fallback


def discover_names(env, target_bowl, debug=False):
    """Find MuJoCo body name for target bowl and cabinet top_side site."""
    ...
    # Find target bowl body
    bowl_body = None
    for n in body_names:
        if target_bowl in n.lower():
            bowl_body = n
            break
    ...
    # Fallback: search for cabinet top site
    ...
    if bowl_body is None:
        raise RuntimeError(f"Could not find {target_bowl} body in: {body_names}")
    if cabinet_site is None:
        raise RuntimeError(f"Could not find cabinet top site in: {site_names}")

    return bowl_body, cabinet_site


def choose_grasp_direction(env, bowl_pos, target_bowl, bowl_radius=0.048, debug=False):
    """
    ...
    # Gather obstacle positions (table-height bodies, excluding target bowl/robot/fixtures)
    obstacle_positions = []
    for i in range(env.sim.model.nbody):
...
def build_place_waypoints(place_xy, cabinet_z, init_rot, place_offset=0.02):
    ...
    place_xy is adjusted so the bowl center (not gripper) lands on cabinet center.
    Each waypoint: (target_pose_4x4, gripper, dist_threshold, hold_steps)
    """
    px, py = place_xy
    cz = cabinet_z
    ...
    return [
        # 5. Move above cabinet top (gripper closed)
        (make_pose(np.array([px, py, cz + 0.10]), rot), +1, 0.03, 0),
...
def collect_one_demo(env, bowl_body, cabinet_site, target_bowl, max_steps, debug=False,
    ...
    cabinet_top_pos = get_site_pos(env, cabinet_site)
    ...
    rim_dx, rim_dy, needs_rot, dir_label = choose_grasp_direction(
        env, bowl_pos, target_bowl, bowl_radius=bowl_radius, debug=debug)

    if debug:
        eef_pose = get_eef_pose(env)
        eef_pos, _ = unmake_pose(eef_pose)
        bowl_site_pos = get_site_pos(env, f"{target_bowl}_default_site")
    ...
    place_xy = cabinet_top_pos[:2] - grasp_offset_xy  # bowl center lands on cabinet center

    if debug:
        print(f"\n  Grasp offset (bowl-eef): [{grasp_offset_xy[0]:.4f}, {grasp_offset_xy[1]:.4f}]")
        print(f"  Adjusted place XY: [{place_xy[0]:.4f}, {place_xy[1]:.4f}] "
              f"(cabinet center: [{cabinet_top_pos[0]:.4f}, {cabinet_top_pos[1]:.4f}])")

    place_waypoints = build_place_waypoints(place_xy, cabinet_top_pos[2], init_rot,
    ...
    if total_steps < max_steps:
        ...
        while total_steps < max_steps:
            ...
            if reached:
                held += 1
                if held >= 10:  # stabilize bowl after horizontal travel
                    break
            if steps_at_wp > 300:
                if debug:
        ...
        if debug:
            ...
            bowl_after_transit = get_body_pos(env, bowl_body)
            print(f"    Transit done: eef={eef_after_transit}, "
                  f"bowl={bowl_after_transit}, steps={steps_at_wp}")
...
EXPECTED_MAX_STEPS = 130  # Calibrated: max=121, ceil/5×5+5=130

def main():
    parser = argparse.ArgumentParser(
        description="Scripted waypoint demo collection (Task 44: black bowl → cabinet)")
    ...
    parser.add_argument("--place_offset", type=float, default=0.02,
                        help="Z offset above cabinet top for place (tune this)")
    parser.add_argument("--bowl_radius", type=float, default=0.048,
                        help="Bowl radius for edge grasp offset (actual rim ~0.047)")
    ...
    target_bowl = get_target_bowl_from_bddl(bddl_path)

    num_demos = 1 if args.debug else args.num
    target = 1 if args.debug else args.target

    print("=" * 60)
    print("Scripted Waypoint Demo Collection (Task 44)")
    print("=" * 60)
    ...
    print(f"Target bowl: {target_bowl}")
    ...
    env.reset()
    bowl_body, cabinet_site = discover_names(env, target_bowl, debug=args.debug)
    ...
    for i in range(num_demos):
        ...
        states, actions, success = collect_one_demo(
            env, bowl_body, cabinet_site, target_bowl,

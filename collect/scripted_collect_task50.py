...
Task 50: pick up the ramekin and place it on the cabinet.
Strategy: edge grasp on ramekin rim with automatic direction selection (avoids nearby obstacles).
  Prefers Y-axis (no rotation), falls back to X-axis (90 Z rotation) if blocked.
  Re-reads ramekin position after WP1 for accurate descend.
  Transit waypoint separates horizontal/vertical movement (table->cabinet is far).
Tuned params: ramekin_radius=0.035, grasp_offset=0.01, place_offset=0.02.
"""
...
def discover_names(env, debug=False):
    """Find MuJoCo body names for ramekin and cabinet site."""
    ...
    # Find cabinet top_side site
    cabinet_site = None
    for n in site_names:
        if "cabinet" in n.lower() and "top_side" in n.lower():
            cabinet_site = n
            break
    if cabinet_site is None:
        for n in site_names:
            if "cabinet" in n.lower() and "top" in n.lower():
                cabinet_site = n
                break

    if ramekin_body is None:
        raise RuntimeError(f"Could not find ramekin body in: {body_names}")
    if cabinet_site is None:
        raise RuntimeError(f"Could not find cabinet site in: {[n for n in site_names if 'cabinet' in n.lower()]}")

    return ramekin_body, cabinet_site


def choose_grasp_direction(env, obj_pos, target_object, radius=0.035, debug=False):
    """
    Choose safe grasp direction from {+Y, -Y, +X, -X}.

    Detection: obstacles within radius*2 of grasp point block that direction.
    Selection: Y-axis first (no rotation), then closest to robot.
    """
    detection_radius = radius * 2  # one diameter

    # Gather obstacle positions (table-height bodies, excluding target/robot/fixtures)
    obstacle_positions = []
    for i in range(env.sim.model.nbody):
...
def build_grasp_waypoints(obj_pos, init_rot, rim_offset_x=0, rim_offset_y=None,
                          radius=0.035, grasp_offset=0.01, needs_rotation=False):
    """
    Build grasp waypoints (1-4): approach, descend, close, lift.

    rim_offset_x/y: offset from object center to rim grasp point.
    ...
    if rim_offset_y is None:
        rim_offset_y = -radius  # default: -Y for backwards compat
    rim_x = ox + rim_offset_x
    rim_y = oy + rim_offset_y
    ...
    return [
        ...
        # 4. Lift up (gripper closed) — 0.28 clears cabinet top (z~1.127)
        (make_pose(np.array([rim_x, rim_y, grasp_z + 0.28]), rot), +1, 0.01, 0),
    ]


def build_place_waypoints(place_xy, cabinet_z, init_rot, place_offset=0.02):
    """
    Build place waypoints (5-8): move above, descend, open, lift.

    place_xy is adjusted so the ramekin center (not gripper) lands on cabinet center.
    Each waypoint: (target_pose_4x4, gripper, dist_threshold, hold_steps)
    """
    px, py = place_xy
    cz = cabinet_z
    ...
    return [
        # 5. Move above cabinet top (gripper closed) — lower to avoid upward motion after transit
        (make_pose(np.array([px, py, cz + 0.05]), rot), +1, 0.03, 0),
        # 6. Descend to place position (gripper closed)
        (make_pose(np.array([px, py, cz + place_offset]), rot), +1, 0.02, 0),
        # 7. Open gripper (release, hold 5 steps)
        (make_pose(np.array([px, py, cz + place_offset]), rot), -1, 999, 5),
    ]


def collect_one_demo(env, ramekin_body, cabinet_site, max_steps, debug=False,
                     place_offset=0.02, ramekin_radius=0.035, grasp_offset=0.01):
    ...
    ramekin_pos = get_body_pos(env, ramekin_body)
    cabinet_pos = get_site_pos(env, cabinet_site)
    ...
    rim_dx, rim_dy, needs_rot, dir_label = choose_grasp_direction(
        env, ramekin_pos, "ramekin", radius=ramekin_radius, debug=debug)

    if debug:
        eef_pose = get_eef_pose(env)
        eef_pos, _ = unmake_pose(eef_pose)
        ramekin_site_pos = get_site_pos(env, "glazed_rim_porcelain_ramekin_1_default_site")
        table_top_pos = get_site_pos(env, "table_top")
        print(f"\n  EEF position:         {eef_pos}")
        print(f"  EEF init rotation:\n{init_rot}")
        print(f"  Ramekin body pos:     {ramekin_pos}")
        print(f"  Ramekin site (dflt):  {ramekin_site_pos}")
        print(f"  Cabinet pos:          {cabinet_pos}")
        print(f"  Table top Z:          {table_top_pos[2]:.4f}")
        print(f"  Ramekin Z (body):     {ramekin_pos[2]:.4f}")
        print(f"  Ramekin Z (site):     {ramekin_site_pos[2]:.4f}")
        print(f"  Cabinet Z:            {cabinet_pos[2]:.4f}")
        print(f"  Ramekin radius:       {ramekin_radius}")
        print(f"  Grasp offset:         {grasp_offset}")
        rim_x = ramekin_pos[0] + rim_dx
        rim_y = ramekin_pos[1] + rim_dy
        grasp_z = ramekin_pos[2] + grasp_offset
    ...
    # --- Phase 1a: WP1 only (approach above ramekin) ---
    # Use initial ramekin_pos for WP1 (move above ramekin, no contact)
    wp1_waypoints = build_grasp_waypoints(ramekin_pos, init_rot,
                                          rim_offset_x=rim_dx, rim_offset_y=rim_dy,
                                          radius=ramekin_radius,
    ...
    for wp_idx, (target_pose, gripper, dist_thresh, hold_steps) in enumerate(wp1_waypoints):
        ...
        if debug:
            ...
            ramekin_after = get_body_pos(env, ramekin_body)
            print(f"    WP{wp_idx+1} done: eef={eef_after}, "
                  f"ramekin={ramekin_after}, steps={steps_at_wp}")
    ...
    # --- Re-read ramekin position after WP1 (ramekin has had 10+~20 steps to settle) ---
    ramekin_pos = get_body_pos(env, ramekin_body)
    if debug:
        print(f"  Ramekin pos after WP1: {ramekin_pos} (z={ramekin_pos[2]:.4f})")

    # --- Phase 1b: WP2-4 (descend, close, lift) with updated ramekin_pos ---
    wp234_waypoints = build_grasp_waypoints(ramekin_pos, init_rot,
                                            rim_offset_x=rim_dx, rim_offset_y=rim_dy,
                                            radius=ramekin_radius,
                                            grasp_offset=grasp_offset,
                                            needs_rotation=needs_rot)[1:]
    ...
    ramekin_after_grasp = get_body_pos(env, ramekin_body)
    eef_after_grasp, _ = unmake_pose(get_eef_pose(env))
    grasp_offset_xy = ramekin_after_grasp[:2] - eef_after_grasp[:2]
    place_xy = cabinet_pos[:2] - grasp_offset_xy  # ramekin center lands on cabinet center

    if debug:
        print(f"\n  Grasp offset (ramekin-eef): [{grasp_offset_xy[0]:.4f}, {grasp_offset_xy[1]:.4f}]")
        print(f"  Adjusted place XY: [{place_xy[0]:.4f}, {place_xy[1]:.4f}] "
              f"(cabinet center: [{cabinet_pos[0]:.4f}, {cabinet_pos[1]:.4f}])")

    place_waypoints = build_place_waypoints(place_xy, cabinet_pos[2], init_rot,
                                            place_offset=place_offset)

    # --- Transit: move horizontally to above cabinet at lifted height ---
    # Table to cabinet requires long travel; separating horizontal/vertical
    # movement prevents pendulum swing that causes placement inaccuracy.
    transit_z = eef_after_grasp[2]  # stay at lift height
    transit_target = make_pose(np.array([place_xy[0], place_xy[1], transit_z]), init_rot)
    transit_pos, _ = unmake_pose(transit_target)

    if total_steps < max_steps:
        if debug:
            print(f"  Transit WP: target={transit_pos}, gripper=close, hold=10")
        steps_at_wp = 0
        held = 0
        reached = False
        while total_steps < max_steps:
            state = env.sim.get_state().flatten()
            action = compute_action(env, transit_target, +1)
            obs, reward, done, info = env.step(action)
            states.append(state)
            actions.append(action)
            total_steps += 1
            ...
            eef_pose = get_eef_pose(env)
            eef_pos, _ = unmake_pose(eef_pose)
            dist = np.linalg.norm(eef_pos - transit_pos)

            if not reached and dist < 0.03:
                reached = True
            if reached:
                held += 1
                if held >= 10:  # stabilize ramekin after horizontal travel
                    break
            if steps_at_wp > 300:
                if debug:
                    print(f"    Transit WP timeout at step {total_steps}, dist={dist:.4f}")
                break

        if debug:
            eef_after_transit, _ = unmake_pose(get_eef_pose(env))
            ramekin_after_transit = get_body_pos(env, ramekin_body)
            print(f"    Transit done: eef={eef_after_transit}, "
                  f"ramekin={ramekin_after_transit}, steps={steps_at_wp}")

    if total_steps < max_steps:
        for wp_idx_offset, (target_pose, gripper, dist_thresh, hold_steps) in enumerate(place_waypoints):
            wp_idx = wp_idx_offset + 4  # 4 grasp waypoints (WP1-4) + transit
            target_pos, _ = unmake_pose(target_pose)
            if debug:
                print(f"  Waypoint {wp_idx + 1}: target={target_pos}, "
                      f"gripper={'close' if gripper > 0 else 'open'}, "
...
EXPECTED_MAX_STEPS = 120  # Calibrated: max=114, ceil/5×5+5=120

def main():
    parser = argparse.ArgumentParser(
        description="Scripted waypoint demo collection (Task 50: ramekin -> cabinet)")
    ...
    parser.add_argument("--place_offset", type=float, default=0.02,
                        help="Z offset above cabinet for place (tune this)")
    parser.add_argument("--ramekin_radius", type=float, default=0.035,
                        help="Ramekin radius for edge grasp offset (default 0.035)")
    parser.add_argument("--grasp_offset", type=float, default=0.01,
                        help="Z offset above ramekin body center for grasp (default 0.01)")
    ...
    print("=" * 60)
    print("Scripted Waypoint Demo Collection (Task 50)")
    print("=" * 60)
    ...
    print(f"Ramekin radius: {args.ramekin_radius}")
    ...
    env.reset()
    ramekin_body, cabinet_site = discover_names(env, debug=args.debug)
    print(f"  Ramekin body:  {ramekin_body}")
    print(f"  Cabinet site:  {cabinet_site}")
    ...
    for i in range(num_demos):
        ...
        states, actions, success = collect_one_demo(
            env, ramekin_body, cabinet_site,
            max_steps=args.max_steps,
            debug=args.debug,
            place_offset=args.place_offset,
            ramekin_radius=args.ramekin_radius,

...
Task 54: put the cream cheese on the stove.
Strategy: center top-down grasp with fixed GRASP_ROT orientation.
Based on Task 79 (cream cheese → plate), modified target to stove cook_region.
"""
...
def discover_names(env, debug=False):
    """Find MuJoCo body name for cream cheese and stove cook_region site."""
...
def build_waypoints(object_pos, stove_pos, place_offset=0.02, grasp_offset=0.01):
    """
    Build waypoint sequence for center top-down grasp.

    Args:
        object_pos: cream cheese center position
        stove_pos: stove cook_region position
    ...
    waypoints = [
        ...
        # 5. Move above stove center (gripper closed)
...
def collect_one_demo(env, cheese_body, stove_site, max_steps, debug=False,
                     place_offset=0.02):
    ...
    stove_pos = get_site_pos(env, stove_site)
    ...
    if debug:
        ...
        print(f"  Stove cook pos:       {stove_pos}")
        print(f"  Object Z:             {object_pos[2]:.4f}")
        print(f"  Stove Z:              {stove_pos[2]:.4f}")
        print(f"  Table top Z:          {table_top_pos[2]:.4f}")

    waypoints = build_waypoints(object_pos, stove_pos, place_offset=place_offset)
...
EXPECTED_MAX_STEPS = 100  # Calibrated: max=92, ceil/5×5+5=100

def main():
    parser = argparse.ArgumentParser(
        description="Scripted waypoint demo collection (Task 54: cream cheese → stove)")
    ...
    print("=" * 60)
    print("Scripted Waypoint Demo Collection (Task 54)")
    print("=" * 60)
    ...
    env.reset()
    cheese_body, stove_site = discover_names(env, debug=args.debug)

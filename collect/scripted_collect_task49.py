...
Task 49: pick up the ramekin and place it on the plate.
...
def discover_names(env, debug=False):
    """Find MuJoCo body names for ramekin and plate."""
    ...
    # Find plate default site
    plate_site = None
    for n in site_names:
        if "plate_1" in n.lower() and "default" in n.lower():
            plate_site = n
            break
    if plate_site is None:
        for n in site_names:
            if "plate_1" in n.lower():
                plate_site = n
                break
    ...
    if plate_site is None:
        raise RuntimeError(f"Could not find plate site in: {[n for n in site_names if 'plate' in n.lower()]}")

    return ramekin_body, plate_site
...
def build_place_waypoints(place_xy, place_z, init_rot, place_offset=0.02):
    """
    Build place waypoints (5-8): move above, descend, open, lift.

    place_xy is adjusted so the ramekin center (not gripper) lands on plate center.
    Each waypoint: (target_pose_4x4, gripper, dist_threshold, hold_steps)
    """
    px, py = place_xy
    pz = place_z
    ...
    return [
        # 5. Move above plate (gripper closed) - relaxed threshold for long travel
        (make_pose(np.array([px, py, pz + 0.10]), rot), +1, 0.03, 0),
        # 6. Descend to place position (gripper closed)
        (make_pose(np.array([px, py, pz + place_offset]), rot), +1, 0.02, 0),
        # 7. Open gripper (release, hold 5 steps)
        (make_pose(np.array([px, py, pz + place_offset]), rot), -1, 999, 5),
    ]


def collect_one_demo(env, ramekin_body, plate_site, max_steps, debug=False,
    ...
    plate_pos = get_site_pos(env, plate_site)
    ...
    place_xy = plate_pos[:2] - grasp_offset_xy  # ramekin center lands on plate center

    if debug:
        print(f"\n  Grasp offset (ramekin-eef): [{grasp_offset_xy[0]:.4f}, {grasp_offset_xy[1]:.4f}]")
        print(f"  Adjusted place XY: [{place_xy[0]:.4f}, {place_xy[1]:.4f}] "
              f"(plate center: [{plate_pos[0]:.4f}, {plate_pos[1]:.4f}])")

    place_waypoints = build_place_waypoints(place_xy, plate_pos[2], init_rot,
                                            place_offset=place_offset)

    # --- Transit: move horizontally to above plate at lifted height ---
...
EXPECTED_MAX_STEPS = 105  # Tightened after video review

def main():
    parser = argparse.ArgumentParser(
        description="Scripted waypoint demo collection (Task 49: ramekin -> plate)")
    ...
    print("=" * 60)
    print("Scripted Waypoint Demo Collection (Task 49)")
    print("=" * 60)
    ...
    env.reset()
    ramekin_body, plate_site = discover_names(env, debug=args.debug)
    print(f"  Ramekin body: {ramekin_body}")
    print(f"  Plate site:   {plate_site}")

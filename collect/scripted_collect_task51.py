...
Uses a sequence of waypoints to pick up the cookie box and place it on the plate.
Based on scripted_collect_task49.py (Task 49: ramekin→plate), modified for Task 51.
Key difference: center top-down grasp (box) instead of edge grasp (bowl).
...
def discover_names(env, debug=False):
    """Find MuJoCo body names for cookie box and plate."""
    ...
    # Find cookie box body
    cookie_body = None
    for n in body_names:
        if "cookies" in n.lower():
            cookie_body = n
            break
    ...
    if cookie_body is None:
        raise RuntimeError(f"Could not find cookie body in: {body_names}")
    if plate_body is None:
        raise RuntimeError(f"Could not find plate body in: {body_names}")

    return cookie_body, plate_body
...
def build_waypoints(object_pos, plate_pos, table_height,
    ...
    Center top-down grasp strategy: gripper descends straight onto the box center
    ...
    Args:
        object_pos: cookie box center position
    ...
    waypoints = [
        # 1. Move above cookie box center + rotate gripper (gripper open)
        (make_pose(np.array([grasp_x, grasp_y, grasp_z + 0.10]), GRASP_ROT), -1, 0.01, 0),
        # 2. Descend to grasp position (gripper open, tight threshold for thin box)
        (make_pose(np.array([grasp_x, grasp_y, grasp_z]), GRASP_ROT), -1, 0.005, 0),
        # 3. Close gripper (hold 15 steps — cookie box is thin, needs longer grip time)
        (make_pose(np.array([grasp_x, grasp_y, grasp_z]), GRASP_ROT), +1, 999, 15),
...
def collect_one_demo(env, cookie_body, plate_body, max_steps, debug=False,
                     place_offset=0.02):
    ...
    # Do NOT record settling steps — they are not part of the task trajectory
    ...
    object_pos = get_body_pos(env, cookie_body)
    ...
    if debug:
        ...
        # Also check cookie default_site position (object center after settling)
        cookie_site_pos = get_site_pos(env, "cookies_1_default_site")
        print(f"\n  EEF position:         {eef_pos}")
        print(f"  Cookie body pos:      {object_pos}")
        print(f"  Cookie site (dflt):   {cookie_site_pos}")
        print(f"  Plate pos:            {plate_pos}")
        print(f"  Cookie Z (body):      {object_pos[2]:.4f}")
        print(f"  Cookie Z (site):      {cookie_site_pos[2]:.4f}")
    ...
    for wp_idx, (target_pose, gripper, dist_thresh, hold_steps) in enumerate(waypoints):
        ...
        if debug:
            ...
            cookie_after = get_body_pos(env, cookie_body)
            print(f"    WP{wp_idx+1} done: eef={eef_after}, "
                  f"cookie={cookie_after}, steps={steps_at_wp}")
...
EXPECTED_MAX_STEPS = 115  # Calibrated: max=106, ceil/5×5+5=115

def main():
    parser = argparse.ArgumentParser(
        description="Scripted waypoint policy demo collection (Task 51: cookie box → plate)")
    ...
    print("=" * 60)
    print("Scripted Waypoint Demo Collection (Task 51)")
    print("=" * 60)
    ...
    env.reset()
    cookie_body, plate_body = discover_names(env, debug=args.debug)
    print(f"  Cookie box body: {cookie_body}")

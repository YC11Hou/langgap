...
Task 40: pick up the black bowl on the right of the ramekin and place it on the plate.
...
def discover_names(env, debug=False):
    """Find MuJoCo body name for akita_black_bowl_2 and plate default site."""
    ...
    # Find akita_black_bowl_2 body (not bowl_1)
    bowl_body = None
    for n in body_names:
        if "akita_black_bowl_2" in n.lower():
            bowl_body = n
            break
    ...
    if bowl_body is None:
        raise RuntimeError(f"Could not find akita_black_bowl_2 body in: {body_names}")
    if plate_site is None:
...
def choose_grasp_direction(env, bowl_pos, bowl_radius=0.048, debug=False):
    ...
    # Gather obstacle positions (table-height bodies, excluding bowl_2/robot/fixtures)
    obstacle_positions = []
    for i in range(env.sim.model.nbody):
        ...
        if any(skip in name.lower() for skip in
            ...
            continue
        if 'akita_black_bowl_2' in name.lower():
            continue
...
def collect_one_demo(env, bowl_body, plate_site, max_steps, debug=False,
    ...
    if debug:
        ...
        bowl_site_pos = get_site_pos(env, "akita_black_bowl_2_default_site")
    ...
    # Prevents collision with intermediate objects during carry.
...
EXPECTED_MAX_STEPS = 130  # Short table task — calibrate after debug run

def main():
    parser = argparse.ArgumentParser(
        description="Scripted waypoint demo collection (Task 40: black bowl right of ramekin → plate)")
    ...
    print("=" * 60)
    print("Scripted Waypoint Demo Collection (Task 40)")
    print("=" * 60)
    ...
    env.reset()
    bowl_body, plate_site = discover_names(env, debug=args.debug)

...
Task 10: pick up the black bowl between the plate and the ramekin and place it on the plate.
...
Tuned params: bowl_radius=0.048, grasp_offset=0.03, place_offset=0.04.
"""
    ...
    if match:
        ...
        for obj in objects:
            if "akita_black_bowl" in obj:
                return obj
    return "akita_black_bowl_2"  # fallback


def discover_names(env, target_bowl, debug=False):
    """Find MuJoCo body name for target bowl and plate default site."""
    ...
    # Fallback: search for plate site
    ...
    if plate_site is None:
        raise RuntimeError(f"Could not find plate_1 site in: {site_names}")

    return bowl_body, plate_site
...
def build_place_waypoints(place_xy, plate_z, init_rot, place_offset=0.02):
    """
    Build place waypoints (5-8): move above, descend, open, lift.

    place_xy is adjusted so the bowl center (not gripper) lands on plate center.
    Each waypoint: (target_pose_4x4, gripper, dist_threshold, hold_steps)
    """
    px, py = place_xy
    sz = plate_z
    ...
    return [
        ...
        # 7. Open gripper (release, hold 10 steps)
        (make_pose(np.array([px, py, sz + place_offset]), rot), -1, 999, 10),
        # 8. Lift away (gripper open)
        (make_pose(np.array([px, py, sz + 0.15]), rot), -1, 0.01, 0),
    ]


def collect_one_demo(env, bowl_body, plate_site, target_bowl, max_steps, debug=False,
                     place_offset=0.04, bowl_radius=0.048, grasp_offset=0.03):
    ...
    place_xy = plate_pos[:2] - grasp_offset_xy  # bowl center lands on plate center
    ...
    # Bowl on cabinet requires long travel to plate; separating horizontal/vertical
    # movement prevents pendulum swing that causes placement inaccuracy.
    transit_z = eef_after_grasp[2]  # stay at lift height (~1.33)
...
def main():
    parser = argparse.ArgumentParser(
        description="Scripted waypoint demo collection (Task 10: black bowl between plate/ramekin → plate)")
    ...
    parser.add_argument("--place_offset", type=float, default=0.04,
    ...
    print("=" * 60)
    print("Scripted Waypoint Demo Collection (Task 10)")
    print("=" * 60)
    ...
    env.reset()
    bowl_body, plate_site = discover_names(env, target_bowl, debug=args.debug)
    ...
    for i in range(num_demos):
        ...
        states, actions, success = collect_one_demo(
            env, bowl_body, plate_site, target_bowl,
